        ...
        try:
            ...
            raise
                    ...
                    if actual_dim != self.dimension:
                        logger.warning(
                            f"Model dimension ({actual_dim}) doesn't match config ({self.dimension}). "
    ...
    async def generate_embeddings_batch(self, texts: List[str]) -> List[EmbeddingResult]:
        ...
        try:
            ...
            if non_empty_texts:
                ...
                embeddings_array = self.model.encode(non_empty_texts, convert_to_numpy=True)
                
                # Create results for non-empty texts
                for i, (text_idx, text) in enumerate(zip(text_indices, non_empty_texts)):
                    embedding_vector = embeddings_array[i].tolist()
                    results[text_idx] = EmbeddingResult(
                        text=text,
                        embedding=embedding_vector,
                        model_name=self.model_name,
                        dimension=self.dimension
